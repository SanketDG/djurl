...
`mypy` brings static type checking with Python!

`django-stubs` brings type hints for Django.

Let's install them using poetry as `dev-dependencies`:

```
$ poetry install -D mypy django-stubs
```

### Configure type hints for Django in mypy using `django-stubs`
...
The boilerplate generated by Django is mostly _Black_ friendly except for
_Black_'s opinion to use double-quoted strings, in contrast to all of
Django's boilerplate code being single-quoted.
